...
def state():
            ...
            events.append(
                {
                    ...
                    "stimulus_ms": nr.get("stimulus_ms", 0),
                    "window_ms": nr.get("window_ms", 500),
                    "streaming": r.get("streaming", False),
                    "delivered_feedback": nr.get("delivered_feedback"),
                    ...
                    "reason_code": r["execution"].get("reason_code"),
    ...
    return {
        "run": RUN.name,
        "now": time.time(),
        "animations_available": all((ANIM_DIR / (name + ".mp4")).is_file() for name in ("buy", "sell_profit", "sell_loss", "hold")),
        "events": events,
        "ledger": ledger(),
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if route == "/stream.json":
            path = RUN / "stream.json"
            payload = json.loads(path.read_text()) if path.exists() else None
            return self.send(json.dumps({"now": time.time(), "stream": payload}).encode(), "application/json")
        if route == "/counts.bin":
            cp = checkpoint()
